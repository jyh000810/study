...
flowchart TD
    ...
    ItemCheck -- "품목 누락" --> MatchAmount["금액 일치 데이터<br/>옵션 제공 (Max 5)"]
    ItemCheck -- "금액 누락" --> MatchItem["품목 유사도 >= 0.7<br/>옵션 제공"]
    ...
    Router -- "GUIDE (가이드)" --> ShowGuide["가이드 답변 제공<br/>+ 발행 유도"]
    Router -- "OTHER (기타)" --> ChitChat["일상 응대<br/>+ 발행 유도"]
